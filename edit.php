...
if(isset($_POST['salvar'])) {
    ...
    if (isset($_FILES["image"]) && $_FILES["image"] ["error"] == 0){
        ...
        $produto->setImagem( uniqid() . $_FILES["image"]["name"]);
        $image = imagecreatefromjpeg($_FILES["image"]["tmp_name"]);
        $image = imagescale($image, 300,  200);
        imagejpeg($image, $produto->getImagemPath());

        //move_uploaded_file($_FILES["image"]["tmp_name"], $produto->getImagemPath());
    }
        ...
        <form method="post"  enctype="multipart/form-data">
            ...
            <div class="input-group">
                ...
                <textarea name="description" id="description" cols="30" rows="10"
                    placeholder="Descrição do produto"> <?= $produto->getDescricao()?></textarea>
            </div>
            <div class="input-group">
